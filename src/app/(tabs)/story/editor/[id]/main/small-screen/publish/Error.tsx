...
const Error = ({ onClose }: { onClose: () => void }) => {
    ...
    return (
        <div className="w-[400px] rounded-t-xl bg-[#333] overflow-hidden">
            ...
            <div className="*:py-2 *:px-1 px-5 py-2 font-sfr tracking-[1px] text-neutral-300 *:border-b *:border-[#444]">
                <p>{posterError}</p>
                <p>{pagesError}</p>
                <p>{ratedError}</p>
                <p>{languageError}</p>
                <p>{summaryError}</p>
                <p className="!border-none">{genresError}</p>
            </div>
        </div>
    );
};
