...
const Edit = ({ characterId, open, onClose }: Props) => {
    ...
    useEffect(() => {
        const currectCharacter = story?.characters?.find(
            (c) => c._id!.toString() === characterId
        );
        currectCharacter &&
            setData({
                name: currectCharacter.name,
                about: currectCharacter.about || "",
            });
    }, []);

    const edit = useEditCharacter(id, characterId);
